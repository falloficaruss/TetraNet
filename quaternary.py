...
class QuaternaryQuantizeFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_c_mask = torch.where((x > 0.0) & (x <= boundary), torch.ones_like(x), grad_c_mask)
        grad_c_mask = torch.where((x <= 0.0) & (x > -boundary), -torch.ones_like(x), grad_c_mask)
...
def quaternary_weight_quantize(weight, c, threshold=1.0):
    ...
    quantized_scaled_weight = QuaternaryQuantizeFunction.apply(scaled_weight, c, torch.tensor(threshold, dtype=weight.dtype, device=weight.device))
...
class QBitLinearQuaternary(nn.Linear):
    ...
    """
    def __init__(self, in_features, out_features, bias=False, initial_c=0.375, threshold=1.0):
    ...
    def forward(self, x):
        # Quantize weights
        quantized_weight = quaternary_weight_quantize(self.weight, self.c, self.threshold)
